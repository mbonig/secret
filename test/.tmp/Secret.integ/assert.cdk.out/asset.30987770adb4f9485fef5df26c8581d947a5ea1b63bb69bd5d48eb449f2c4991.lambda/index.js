...
async function createSecretValue({ generateSecretString, secretArn }) {
  ...
  if (generateSecretString.generateStringKey) {
    const randomValue = await client.send(new import_client_secrets_manager2.GetRandomPasswordCommand({
  ...
  await client.send(new import_client_secrets_manager2.PutSecretValueCommand({
    SecretId: secretArn,
    SecretString: JSON.stringify(secretValue)
  }));
}

// src/EventHandler/updateSecretValue.ts
var import_client_secrets_manager3 = require("@aws-sdk/client-secrets-manager");

// src/EventHandler/didRandomPropertiesChange.ts
...
async function updateSecretValue({ oldGenerateSecretString, generateSecretString, secretArn }) {
  const existingSecretValue = await client.send(new import_client_secrets_manager3.GetSecretValueCommand({
    SecretId: secretArn
  }));
  let parsedValue = {};
  try {
  ...
  if (didRandomPropertiesChange(oldGenerateSecretString, generateSecretString) || generateSecretString.generateStringKey !== oldGenerateSecretString.generateStringKey) {
    const { RandomPassword: newRandomValue } = await client.send(new import_client_secrets_manager3.GetRandomPasswordCommand({
  ...
  await client.send(new import_client_secrets_manager3.PutSecretValueCommand({
    SecretId: secretArn,
